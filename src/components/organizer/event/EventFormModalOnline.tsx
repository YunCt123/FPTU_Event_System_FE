...
const EventFormModalOnline: React.FC<Props> = ({
  event,
  onClose,
  onSuccess,
}) => {
  ...
  const [originalData, setOriginalData] = useState<BookingOnlineRequest | null>(
    null
  );
    ...
    const fetchEventDetails = async () => {
      if (event) {
        try {
          ...
          console.log("Event speakers from API:", fullEvent?.eventSpeakers);
          ...
          const formattedData = {
            ...
            staffIds: (fullEvent?.eventStaffs ?? []).map(
              (s: any) => s.userId
            ) as number[],
            speakers: (fullEvent?.eventSpeakers ?? []).map((s: any) => {
              const speakerId = s.speakerId ?? s.speaker?.id ?? s.id ?? 0;
              const topic = s.topic ?? "";
              console.log("Mapping speaker:", { s, speakerId, topic });
              return {
                speakerId: speakerId,
                topic: topic,
              };
          ...
          const fallbackData = {
            ...
            staffIds: (ev?.eventStaffs ?? ev?.staffIds ?? []).map(
              (s: any) => s.userId ?? s
            ) as number[],
            speakers: (ev?.eventSpeakers ?? ev?.speakers ?? []).map(
              (s: any) => ({
                speakerId: s.speakerId ?? s.speaker?.id ?? s.id ?? 0,
                topic: s.topic ?? "",
              })
            ),
            isOnline: true,
  ...
  const handleChange = (
    ...
    setForm((p) => ({
      ...p,
      [name]:
        name === "organizerId" || name === "hostId" ? Number(value) : value,
    }));
  ...
  const toggleStaff = (id: number) => {
    setForm((p) => ({
      ...p,
      staffIds:
        p.staffIds && p.staffIds.includes(id)
  ...
  const addSpeaker = () => {
    setForm((p) => ({
      ...p,
      speakers: [...(p.speakers || []), { speakerId: 0, topic: "" }],
    }));
  };
  const updateSpeaker = (
    idx: number,
    ...
    setForm((p) => {
      ...
      s[idx] = {
        ...(s[idx] as any),
        [key]: key === "speakerId" ? Number(value) : value,
      } as any;
      return { ...p, speakers: s };
    });
  };
  ...
  const handleSubmit = async (ev?: React.FormEvent) => {
    ...
    try {
      if (event && originalData) {
        ...
        if (form.bannerUrl !== originalData.bannerUrl)
          updateData.bannerUrl = form.bannerUrl;
        if (form.startTime !== originalData.startTime)
          updateData.startTime = form.startTime;
        if (form.endTime !== originalData.endTime)
          updateData.endTime = form.endTime;
        if (form.onlineMeetingUrl !== originalData.onlineMeetingUrl)
          updateData.onlineMeetingUrl = form.onlineMeetingUrl;

        // So sánh staffIds (array)
        const staffIdsChanged =
          JSON.stringify(form.staffIds?.sort()) !==
          JSON.stringify(originalData.staffIds?.sort());
        if (staffIdsChanged) {
          updateData.staffIds = form.staffIds || [];
        }

        // So sánh speakers (array of objects)
        const speakersChanged =
          JSON.stringify(
            form.speakers
              ?.map((s) => ({ speakerId: s.speakerId, topic: s.topic }))
        ...
        if (speakersChanged) {
          updateData.speakers =
            form.speakers?.map((s: any) => ({
              speakerId: Number(s.speakerId),
              topic: s.topic,
            })) || [];
        }

        if (form.organizerId !== originalData.organizerId) {
          updateData.organizerId = form.organizerId;
        }

        if (form.hostId !== originalData.hostId) {
        ...
        // CREATE MODE
        const isRecurring =
          form.recurrenceType && form.recurrenceType !== "NONE";

        if (isRecurring) {
          // Use bookingWeekly for recurring online events
          // Online events không cần gửi maxCapacity
          const weeklyData = {
            title: form.title,
            description: form.description,
            category: form.category || undefined,
            bannerUrl: form.bannerUrl?.trim() || undefined, // ✅ Thêm bannerUrl
            startTime: form.startTime,
            endTime: form.endTime,
            startTimeRegister: form.startTime, // Use startTime as registration start
            endTimeRegister: form.endTime, // Use endTime as registration end
            organizerId: form.organizerId,
            hostId: form.hostId, // ✅ Thêm hostId
            venueId: undefined, // Online events don't have venue
            recurrenceType: form.recurrenceType || "WEEKLY",
            recurrenceInterval: Number(form.recurrenceInterval) || 1,
            recurrenceCount: form.recurrenceCount || undefined,
            isOnline: true, // ✅ Thêm isOnline cho online events
            onlineMeetingUrl: form.onlineMeetingUrl, // ✅ Thêm onlineMeetingUrl
            staffIds: form.staffIds || [], // ✅ Thêm staffIds
            speakers:
              form.speakers?.map((s: any) => ({
                speakerId: Number(s.speakerId),
                topic: s.topic,
          ...
          // Cast để bypass maxCapacity requirement cho online events
          const res = await eventService.bookingWeekly(weeklyData as any);
          toast.success("Tạo sự kiện online định kỳ thành công!");
          const data = (res as any)?.data ?? res;
          onSuccess(data as any);
          onClose();
        } else {
          // Use normal bookingOnline for non-recurring events
          const payload: BookingOnlineRequest = {
            title: form.title,
            description: form.description,
            category: form.category || undefined,
            bannerUrl: form.bannerUrl?.trim() || undefined, // ✅ Đảm bảo trim và chỉ gửi nếu có giá trị
            startTime: form.startTime,
            endTime: form.endTime,
            organizerId: form.organizerId,
            hostId: form.hostId,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/90 text-sm mt-1">
                {event
                  ? "Cập nhật thông tin sự kiện trực tuyến"
                  : "Tạo sự kiện dạng trực tuyến"}
              </p>
            </div>
          </div>
          <button
                        ...
                        form.recurrenceType !== "NONE" && (
                          <p className="text-xs text-gray-600">
                            ...
                            {form.recurrenceCount &&
                              `, tổng cộng ${form.recurrenceCount} lần`}
                          </p>
                        )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {(form.speakers || []).map((sp, idx) => (
                      ...
                      >
                        <div className="flex gap-2">
                          ...
                          >
                            ...
                            {currentSpeakerPage < totalSpeakerPages && (
                              ...
                              >
                                Tải thêm speaker (Trang {currentSpeakerPage + 1}
                                /{totalSpeakerPages})
                              </option>
                            )}
                          </select>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateSpeaker(idx, "topic", e.target.value)
                          }
                    ...
                    {isLoadingSpeakers && (
                      <p className="text-xs text-blue-600 text-center">
                        Đang tải danh sách speaker...
                      </p>
                    )}
                    {speakers.length > 0 && (
                      <p className="text-xs text-gray-500 text-center">
                        Đang hiển thị {speakers.length} speaker
                        {currentSpeakerPage < totalSpeakerPages &&
                          ` - Còn ${
                            totalSpeakerPages - currentSpeakerPage
                          } trang nữa`}
                      </p>
                    )}
                  </div>
